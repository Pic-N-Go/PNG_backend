...
public class CommunityPostService {
    ...
    public PostPageResponse getPosts(Long userId, PostSort sort, String keyword, int page, int size) {
        ...
        Page<CommunityPost> result = switch (sort) {
    ...
    public PostResponse createPost(Long userId, PostCreateRequest request, List<MultipartFile> images) {
        ...
        try {
            for (int index = 0; index < images.size(); index++) {
                ...
                CommunityImage image = CommunityImage.uploaded(userId, uploaded.key(), exif);
    ...
    public PostResponse updatePost(Long postId, Long userId, PostUpdateRequest request, List<MultipartFile> newImages) {
        ...
        List<CommunityImage> existingImages = imageRepository.findByPostIdOrderByPostOrderAsc(postId);

        List<MultipartFile> files = newImages == null ? List.of() : newImages;

        validateNewImageFiles(files);

        List<CommunityImage> retainedImages = resolveRetainedImages(existingImages, request.retainedImageIds());
    ...
    private PhotoExifResponse toExifResponse(CommunityImage image) {
    ...
    // 비관적 쓰기 잠금 적용 (동시성 문제)
    private CommunityPost findPostForUpdate(Long postId) {
    ...
    private List<PostResponse> toFeedResponses(List<CommunityPost> posts, Long userId) {
        if (posts.isEmpty()) {
            return List.of();
        }
    ...
    private PostImageResponse toImageResponse(CommunityImage image) {
    ...
    private PostResponse buildResponse(CommunityPost post, List<PostImageResponse> images, boolean liked, boolean bookmarked) {
    ...
    private List<CommunityImage> resolveRetainedImages(
            List<CommunityImage> existingImages,
        ...
        Map<Long, CommunityImage> existingImageMap = existingImages.stream()
                .collect(Collectors.toMap(
                        CommunityImage::getId,
                        image -> image
                ));
        List<CommunityImage> retainedImages = new ArrayList<>();

        for (Long imageId : requestedImageIds) {
            CommunityImage image = existingImageMap.get(imageId);
            if (image == null) {
                throw new CustomException(CommunityErrorCode.POST_IMAGE_INVALID);
            }
    ...
    private void validatePostAuthor(CommunityPost post, Long userId
    ) {

...
public class TourApiSyncDeadLetterConsumer {
    ...
    @RabbitListener(queues = TourApiRabbitMQConfig.DEAD_LETTER_STATUS_QUEUE_NAME)
    public void consumeSpotFailure(TourApiSyncMessage message) {
        String error = "일반 Spot 동기화가 재시도 후 최종 실패했습니다.";
        log.error("[TourApiSyncDeadLetterConsumer] 일반 Spot 동기화 최종 실패: jobId={}", message.jobId());
        syncStatusManager.markStageFailed(message.jobId(), TourApiSyncStatusManager.Stage.SPOT, error);
    }

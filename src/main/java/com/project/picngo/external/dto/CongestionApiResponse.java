...
public record CongestionApiResponse(Response response) {
    ...
    @JsonIgnoreProperties(ignoreUnknown = true)
    public record Items(
            @JsonProperty("item") List<CongestionItem> item
    ) {
        @JsonCreator
        public static Items fromObject(@JsonProperty("item") List<CongestionItem> item) {

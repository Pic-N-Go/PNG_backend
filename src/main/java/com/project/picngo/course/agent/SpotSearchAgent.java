...
public class SpotSearchAgent {
    ...
    public List<SpotCandidate> searchCandidates(PlanGoal goal) {
        ...
        if (spots.isEmpty() && regionCoordinateResolver != null) {
            ...
            if (centerOpt.isPresent()) {
                ...
                // 반경 30km -> 60km -> 100km -> 150km -> 200km 순차 확장
                for (double radiusKm : List.of(30.0, 60.0, 100.0, 150.0, 200.0)) {
                    ...
                    if (!nearbySpots.isEmpty()) {
                        spots = prioritizeCategories(nearbySpots, goal.categories(), limit);
                        log.info("요청 지역({}) 인근 스팟 발굴 성공: 반경 {}km 내 {}건 (카테고리 매칭 반영)",
                                goal.region(), radiusKm, spots.size());
                        break;
                    }
                }

                // 3차-B: 200km 반경 내에서도 스팟이 없으면, 전국(서울) 인기순으로 건너뛰지 않고 최근접 물리적 거리 스팟 조회
                if (spots.isEmpty()) {
                    log.info("요청 지역({}) 반경 200km 내 스팟 부재 -> 최근접 물리적 거리 스팟 조회", goal.region());
                    List<Spot> nearestSpots = spotRepository.findNearestApprovedSpots(
                            center.latitude(),
                            center.longitude(),
                            SpotStatus.APPROVED.name(),
                            limit * 2
                    ...
                    if (!nearestSpots.isEmpty()) {
                        spots = prioritizeCategories(nearestSpots, goal.categories(), limit);
                        log.info("요청 지역({}) 최근접 스팟 발굴 성공: {}건", goal.region(), spots.size());
                    }
                }
            }
        }

        // 4차: 전국 인기 스팟 최종 폴백 (중심 좌표 해석이 불가능한 완전 미지정 요청일 때만)
        if (spots.isEmpty()) {
    ...
    private List<Spot> prioritizeCategories(List<Spot> candidates, List<com.project.picngo.common.domain.SpotCategory> targetCategories, int limit) {
        ...
        List<Spot> matched = new ArrayList<>();
        List<Spot> others = new ArrayList<>();

        for (Spot s : candidates) {
            boolean hasCategory = s.getCategories() != null
                    && s.getCategories().stream().anyMatch(targetCategories::contains);
            if (hasCategory) {
                matched.add(s);
            } else {
                others.add(s);
        ...
        List<Spot> result = new ArrayList<>(matched);
        for (Spot s : others) {
    ...
    public static RegionPair resolveRegionPair(String rawRegion) {
        ...
        // 1. 혼동되기 쉬운 복합/특수 지명 최우선 정규화
        if (r.contains("전남광주") || r.contains("전남 광주") || r.contains("광주광역시")) return new RegionPair("광주", "광주광역시");
        if (r.contains("경기광주") || r.contains("경기 광주")) return new RegionPair("경기", "경기도");

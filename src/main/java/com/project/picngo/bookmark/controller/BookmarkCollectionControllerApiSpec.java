...
public interface BookmarkCollectionControllerApiSpec {
    ...
    @Operation(summary = "컬렉션 목록 조회", description = "유저의 북마크 컬렉션 목록을 반환합니다. spotId를 주면 각 컬렉션에 해당 스팟 소속 여부(contains)를 포함합니다. 최초 조회 시 기본 컬렉션('내 즐겨찾기')이 자동 생성됩니다. 로그인 필요.")
    ...
    @Operation(summary = "컬렉션 생성", description = "이름 + color + icon으로 컬렉션을 생성합니다. 유저당 최대 5개, 이름 20자 이하. color/icon은 허용 키만 가능합니다. 로그인 필요.")
    ResponseEntity<BookmarkCollectionResponse> createCollection(
            @Parameter(hidden = true) @AuthenticationPrincipal CustomUserDetails userDetails,
            @Valid @RequestBody CreateCollectionRequest request
    );

    @Operation(summary = "스팟 소속 컬렉션 동기화", description = "체크된 collectionIds 집합으로 이 스팟의 멤버십을 통째 동기화합니다(빠진 컬렉션은 제거). 빈 배열이면 모든 컬렉션에서 제거됩니다. 로그인 필요.")

...
public interface AdminInquiryControllerApiSpec {
    ...
    @Operation(summary = "관리자용 전체 1:1 문의 목록 및 검색 조회", description = "전체 1:1 문의 목록을 최신순으로 페이징 조회합니다. 문의 유형(FEATURE/BUG/ACCOUNT/SPOT/OTHER), 문의 상태(PENDING/ANSWERED/RESOLVED), 해결 여부(isResolved), 검색어 필터링이 가능합니다.")
    @ApiResponses({
    ...
    ResponseEntity<Page<InquiryResponse>> getInquiriesForAdmin(
            @Parameter(description = "문의 유형 (FEATURE, BUG, ACCOUNT, SPOT, OTHER)", example = "FEATURE") @RequestParam(required = false) InquiryType type,

...
class CourseFacadeTest {
    ...
    void syncCourseSpots_storesNullWhenTimeUnknown() {
        ...
        Spot spot1 = createSpot(100L, 33.1, 126.1);
        // TourAPI 파싱 실패로 (0.0, 0.0)이 저장된 유령 좌표 스팟
        Spot spot2 = createSpot(200L, 0.0, 0.0);
        when(spotRepository.findByIdIn(List.of(100L, 200L))).thenReturn(List.of(spot1, spot2));

        // 길찾기 실패, 추정 계산도 유효 범위 밖이라 null
        when(routeCacheService.getTravelInfoWithCache(33.1, 126.1, 0.0, 0.0)).thenReturn(null);
        when(routeCacheService.calculateFallbackTime(33.1, 126.1, 0.0, 0.0)).thenReturn(null);
